...
const [Level, setLevelItems] = useState([]);
const [Grade, setGradeItems] = useState([]);
const [User, setRegisterItems] = useState([]);
...
useEffect(() => {
 if(data == null )
 {
  ...
  useEffect(() => {
    fetch(config.APP_BASE_URL + "/dashbord")
...
useEffect(() => {
    fetch(config.APP_BASE_URL + "/dashbord/levelcount")
    
        .then(res => res.json())
        .then(
            (result) => {
                setIsLoaded(true);
                setLevelItems(result);
            },
...
useEffect(() => {
    fetch(config.APP_BASE_URL + "/dashbord/gradecount")
    
        .then(res => res.json())
        .then(
            (result) => {
                setIsLoaded(true);
                setGradeItems(result);
            },
...
useEffect(() => {
    fetch(config.APP_BASE_URL + "/dashbord/registerusercount")
    
        .then(res => res.json())
        .then(
            (result) => {
                setIsLoaded(true);
                setRegisterItems(result);
            },
          ...
          <nav class="sb-topnav navbar navbar-expand navbar-dark bg-dark">
              ...
              <ul class="navbar-nav ms-auto ms-md-0 me-3 me-lg-4">
                  <li class="nav-item dropdown">
                      <a class="nav-link dropdown-toggle" onClick={logout} id="navbarDropdown" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false"><i class="fas fa-user fa-fw"></i>Logout</a>
                      ...
                      <div class="container-fluid px-4">
                          ...
                          <div class="row">
                              <div class="col-xl-3 col-md-6">
                                  ...
                                  {Items.map(item => (
                                      <div class="card-body"><h3>{item.questioncount}</h3></div>
                                      ))}
                                      <div class="card-footer d-flex align-items-center justify-content-between">
                                          <a class="small text-white stretched-link" href="/questions">Questions</a>
                              ...
                              <div class="col-xl-3 col-md-6">
                                  ...
                                  {Level.map(item => (
                                      ...
                                      <div class="card-footer d-flex align-items-center justify-content-between">
                                          <a class="small text-white stretched-link" href='/setquiz' >Quiz Blocks</a>
                                          <div class="small text-white"><i class="fas fa-angle-right"></i></div>
                                      </div>
                                  </div>
                              </div>
                              <div class="col-xl-3 col-md-6">
                                  ...
                                  {Grade.map(item => (
                                      ...
                                      <div class="card-footer d-flex align-items-center justify-content-between">
                                          <a class="small text-white stretched-link" href="#">Grade</a>
                                          <div class="small text-white"><i class="fas fa-angle-right"></i></div>
                                      </div>
                                  </div>
                              </div>
